...
run-make-system-partition:
  mkdir -p {{run_directory}}/esp/efi/boot
  cp {{root_directory}}/target/x86_64-unknown-uefi/debug/starlite.efi {{run_directory}}/esp/efi/boot/bootx64.efi

run: build run-mkdir run-copy-files run-make-system-partition
  cd  {{run_directory}} && qemu-system-x86_64 -enable-kvm \
    -drive if=pflash,format=raw,readonly=on,file=OVMF_CODE.fd \
    -drive if=pflash,format=raw,readonly=on,file=OVMF_VARS.fd \
    -drive format=raw,file=fat:rw:esp

run-headless: build run-mkdir run-copy-files run-make-system-partition
